...
REM --- 4. 读取版本号 (src/config/index.ts 的 APP_CONFIG.version) ---
set "VER="
...
REM --- 11. 复制到 app版本 文件夹, 命名为 宝可梦小图鉴_版本号.apk ---
set "OUT_DIR=%~dp0app版本"
if not exist "%OUT_DIR%" mkdir "%OUT_DIR%"
if /i "%FLAVOR%"=="release" (
  set "OUT_NAME=宝可梦小图鉴_%VER%_release.apk"
) else (
  set "OUT_NAME=宝可梦小图鉴_%VER%.apk"
)
copy "%APK%" "%OUT_DIR%\%OUT_NAME%" >nul
if exist "%OUT_DIR%\%OUT_NAME%" (
  echo [OK] 已生成发布文件: %OUT_NAME%
) else (
  echo [WARN] 复制失败, 请手动使用原始产物: %APK%
)
